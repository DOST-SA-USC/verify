...
- Verification data is fetched server-side from `tracker.dostsausc.org`
...
3. Server performs `fetch("https://tracker.dostsausc.org/api/scholar/{id}")`.
4. Request includes `Authorization: Bearer ${process.env.DSU_API_KEY}`.
5. Next.js caching is configured with:
   - `revalidate: 300` seconds
   - cache tag: `scholar:{id}`
6. On success (`res.ok`), app renders `Valid` UI (`src/app/[id]/valid.tsx`).
7. On non-success, app renders `Invalid` UI (`src/app/[id]/invalid.tsx`).
